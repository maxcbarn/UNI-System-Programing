...
struct ParsedLine {
    ...
    REGISTERS_8b    reg8b_dest = A;    // registrador destino (LD A, B -> A)
    REGISTERS_8b    reg8b_src  = A;    // registrador fonte   (LD A, B -> B)
    REGISTERS_16b   reg16b     = AF;   // registrador 16 bits (PUSH BC -> BC)
    REGISTERS_ESP   regEsp     = IX;   // IX ou IY (para indexado)
...
private:
    ...
    static REGISTERS_8b  ToReg8b(const Token& tok);
    static REGISTERS_16b ToReg16b(const Token& tok);
    static REGISTERS_ESP ToRegEsp(const Token& tok);
