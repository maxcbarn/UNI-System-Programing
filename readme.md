...
- AdressingTypes.hpp & AdressingTypes.cpp - This is the base class of the adpaters that will treat the adressing types, it's propurse is to recive the instructions in any adressing type and translate to instruction in the architeture using the accumulator, it's a adapter (not finished)
- Processor.hpp & Processor.cpp - Wrapper of all functions of the processor, interface that will interact with the processor, it's a singleton (not finished)